...
void OrderBook::outputFillEvent(int side, Order *buyOrd, Order *sellOrd)
{
...
void OrderBook::matchOrder(int side)
{
    while(true)
    {
...
void OrderBook::addToBook(int msgType, unsigned long int orderid, int side, unsigned long int quantity, double price)
{
...
void OrderBook::deleteOrderUnit(int side, double price)
{
    // Buy Book.
    if(side == 0)
    {
...
void OrderBook::cancelOrder(int msgType, unsigned long int orderid)
{
    unordered_map<unsigned long int, Order*>::iterator idIterator = idToOrderMap.find(orderid);
